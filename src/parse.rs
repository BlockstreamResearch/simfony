...
impl FunctionParams {
    pub fn to_pattern(&self) -> Pattern {
        match self.0.is_empty() {
            true => Pattern::Ignore,
            false => Pattern::array(self.0.iter().cloned().map(Pattern::Identifier)),
        }
    }
}
...
impl PestParse for Statement {
    fn parse(pair: pest::iterators::Pair<Rule>) -> Self {
        ...
        match inner_pair.as_rule() {
            ...
            Rule::function => Statement::Function(Function::parse(inner_pair)),
...
impl PestParse for FunctionParams {
    fn parse(pair: pest::iterators::Pair<Rule>) -> Self {
        ...
        let params = pair.into_inner().map(Identifier::parse).collect();
        Self(params)
    }
}
